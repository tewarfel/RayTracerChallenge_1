...
def step_impl_generic_tuple(context,item,x,y,z,w):
    try:
        ...
        context.tuple = {}
    context.tuple[item] = base.Vec4(float(x), float(y), float(z), float(w))
...
def step_impl_generic_vector_A(context, item, x_numerator, x_denom, y_numerator, y_denom, z):
    try:
        ...
        context.tuple = {}
    context.tuple[item] = base.vector(np.sqrt(float(x_numerator)) / float(x_denom), np.sqrt(float(y_numerator)) / float(y_denom), float(z))
    ...
    try:
        if (context.tuple is None):
            ...
            if v in context.tuple.keys():
                context.tuple[item] = base.normalize(context.tuple[eval(v)])
            else:
...
def step_impl_when_generic_vector_reflected(context, item, v, n):
    ...
    context.tuple[str(item)] = base.reflect(v_value, n_value)
...
def step_then_item_is_thing(context, item, thing):
    estring = "base.is_" + str(thing) + "(context.tuple['"+str(item)+"'])"
...
def step_then_dot_product_is_value(context, item1, item2, value):
    estring = "base.dot(context.tuple['"+str(item1)+"'], context.tuple['"+str(item2)+"'])"
...
def step_then_tuple_sum(context, item1, index1, item2, index2, x, y, z, w):
    estring = "base.vec4(float(" + str(x) + "), float(" + str(y) + "), float(" + str(z) + "), float(" + str(w) + "))"
    i1 = context.tuple[str(item1)+str(int(index1))]
    i2 = context.tuple[str(item2)+str(int(index2))]
    new_item = base.vec4(i1.x + i2.x, i1.y + i2.y, i1.z + i2.z, i1.w + i2.w)
...
def step_impl_indexed_tuple(context, variable, index, x,y,z,w):
    ...
    value_string = estring = "base.vec4(float(" + str(x) + "), float(" + str(y) + "), float(" + str(z) + "), float(" + str(w)+"))"
...
def step_impl_indexed_point(context, variable, index, x, y, z):
    ...
    assignment_string = "context.tuple." + str(variable) + str(int(index))
    value_string = estring = "base.vec4(float(" + str(x) + "), float(" + str(y) + "), float(" + str(z) + "), 1)"
    context.tuple[str(variable) + str(int(index))] = eval(value_string)
...
def step_then_vect_sub(context, item1, item2, x, y, z):
    estring = "base.vec4(float(" + str(x) + "), float(" + str(y) + "), float(" + str(float(z)) + "), 1.0)"
...
def step_then_vect_sub_zero(context, item1, item2, x, y, z):
    estring = "base.vec4(float(" + str(x) + "), float(" + str(y) + "), float(" + str(float(z)) + "), 0)"
    i1 = context.tuple[str(item1)]
    i2 = context.tuple[str(item2)]
    new_item = base.vec4(i1.x - i2.x, i1.y - i2.y, i1.z - i2.z, i1.w - i2.w)
    assert_that(eval(estring), equal_to(new_item))
...
def step_then_magnitude_equals(context, item, x,y,z):
    v = context.tuple[str(item)]
    norm_v = base.normalize(v)
    assert_that(norm_v, equal_to(base.vector(float(x), float(y), float(z))))
    
@then("normalize({item:TestVariable}) = approximately vector({x},{y},{z})")
def step_then_magnitude_equals(context, item, x,y,z):
    v = context.tuple[str(item)]
    norm_v = base.normalize(v)
    assert_that(base.equal(norm_v.x, float(x)))
    assert_that(base.equal(norm_v.y, float(y)))
    assert_that(base.equal(norm_v.z, float(z)))
    assert_that(base.equal(norm_v.w, 0))
...
def step_then_cross_product_is_value(context, item1, item2, x,y,z):
    estring = "base.cross(context.tuple['"+str(item1)+"'], context.tuple['"+str(item2)+"'])"
    assert_that(eval(estring), equal_to(base.vector(float(x), float(y), float(z))))
...
def step_then_vect_result_sqrt_rational(context, item, xnum, xdenom, ynum, ydenom, znum, zdenom):
    value = base.vec4(np.sqrt(float(xnum)) / float(xdenom), np.sqrt(float(ynum)) / float(ydenom), np.sqrt(float(znum)) / float(zdenom), 0.0)
    i1 = context.tuple[str(item)]
    assert(base.equal(value, i1))
...
def step_then_vect_result(context, item, x, y, z):
    estring = "base.vec4(float(" + str(x) + "), float(" + str(y) + "), float(" + str(float(z)) + "), 0.0)"
    i1 = context.tuple[str(item)]
    assert(base.equal(eval(estring), i1))
